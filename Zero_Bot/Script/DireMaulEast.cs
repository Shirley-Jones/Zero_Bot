...
{
    public class DireMaulEast : DLLManager
    {
        public override string ScriptName => "厄运东法师刷花";
        ...
        public Node _smartPrepTree()
        {
            return new ActionNode(() =>
            {
                ...
                int rubyCount = GetItemCount(_hProcess, 8008);
                int citrineCount = GetItemCount(_hProcess, 8007);

                bool needsWaterItem = waterCount < 20;
                bool needsFoodItem = foodCount < 20;
                bool needsRubyItem = rubyCount == 0;
                bool needsCitrineItem = citrineCount == 0;
                ...
                GetEatDrinkStatus(_hProcess, _playerBase, out bool isEating, out bool isDrinking);
                bool hasFoodBuff = isEating;
                bool hasWaterBuff = isDrinking;
                bool hasAnyBuff = hasFoodBuff || hasWaterBuff;
                bool isFull = hp > 99f && mp > 99f;

                // ========== 2. 跳跃硬直拦截 ==========
                if ((DateTime.Now - _lastStandUpTime).TotalMilliseconds < 2000)
                {
                    _wasEatingOrDrinking = hasAnyBuff;
                    return NodeState.Running;
                }

                // ========== 3. 起立判断（血蓝满主动打断 / Buff自然结束） ==========
                bool needToInterrupt = hasAnyBuff && isFull;
                bool buffNaturallyExpired = _wasEatingOrDrinking && !hasAnyBuff;
                _wasEatingOrDrinking = hasAnyBuff;

                if (needToInterrupt || buffNaturallyExpired)
                {
                    if (needToInterrupt)
                        Logger.Write($"[{CurrentPlayerName}] [备战系统] 血蓝已满，主动打断吃喝，跳跃起立...");
                    else
                ...
                // ========== 4. 最终放行条件 ==========
                if (!needsWaterItem && !needsFoodItem && !needsRubyItem && !needsCitrineItem && isFull)
                {
                ...
                // ========== 5. 施法动作僵直 ==========
                if ((DateTime.Now - _lastCastTime).TotalSeconds < 3.5)
                    return NodeState.Running;

                // ========== 6. 制造业（仅当站立时，即没有吃喝Buff） ==========
                if (!hasAnyBuff)
                {
                    string craftSpell = null;
                    string craftLog = null;

                    if (needsWaterItem && currentMana >= 820)
                    {
                        craftSpell = "造水术";
                        craftLog = $"缺水({waterCount}/20)，施放造水术...";
                    }
                    else if (needsFoodItem && currentMana >= 750)
                    {
                        craftSpell = "造食术";
                        craftLog = $"缺面包({foodCount}/20)，施放造食术...";
                    }
                    else if (needsRubyItem && currentMana >= 1520)
                    {
                        craftSpell = "制造魔法红宝石";
                        craftLog = "缺魔法红宝石(0/1)，施放制造魔法红宝石...";
                    }
                    else if (needsCitrineItem && currentMana >= 1180)
                    {
                        craftSpell = "制造魔法黄水晶";
                        craftLog = "缺魔法黄水晶(0/1)，施放制造魔法黄水晶...";
                    }

                    if (craftSpell != null)
                    {
                        Logger.Write($"[{CurrentPlayerName}] [备战系统] {craftLog}");
                        ExecuteDynamicLua($"CastSpellByName('{craftSpell}');");
                        _lastCastTime = DateTime.Now;
                        return NodeState.Running;
                    }
                }

                // ========== 7. 吃喝恢复逻辑（允许在已有Buff时补充另一个） ==========
                bool needEat = hp <= 99f && foodCount > 0;
                bool needDrink = mp <= 99f && waterCount > 0;
                bool missingFoodBuff = needEat && !hasFoodBuff;
                bool missingWaterBuff = needDrink && !hasWaterBuff;

                bool itemCooldown = (DateTime.Now - _lastItemUseTime).TotalMilliseconds < 500;
                ...
                if (!itemCooldown)
                {
                    ...
                    if (missingWaterBuff)
                    {
                        Logger.Write($"[{CurrentPlayerName}] [备战系统] 蓝量未达标，喝魔法水...");
                        UseItemByItemId(_hProcess, _bestWaterItemId);
                        _lastItemUseTime = DateTime.Now;
                        return NodeState.Running;
                    }
                ...
                {
                    // 物品冷却中，但还在等待使用食物或水，就继续等
                    if (missingFoodBuff || missingWaterBuff)
                        return NodeState.Running;
                }

                // ========== 8. 正在吃喝且无缺失需求，等待恢复 ==========
                if (hasAnyBuff && !missingFoodBuff && !missingWaterBuff)
                    return NodeState.Running;
        ...
        public Node FirstWaveNode()
        {
            ...
            return new Selector(
                ...
                new Sequence(
                    ...
                    new ActionNode(() =>
                    {
                        if (MoveTo(_hProcess, _playerBase, 129.15f, -220.79f, -56.83f, 0.5f))
                        {
                    ...
                    new ActionNode(() =>
                    {
                        ulong nearestGuid = FindNearestUnitGuid(_hProcess, _playerBase, 13022);

                        if (nearestGuid != 0)
                        {
                            // 以十六进制格式输出，与 Lua 脚本风格保持一致
                            Select_Target(_hProcess, nearestGuid);
                            Logger.Write($"[{CurrentPlayerName}] [战斗系统] 找到最近 抽苔者 的 GUID: 0x{nearestGuid:X} 并选中它了");
                        }
                        else
                        {
                            Logger.Write($"[{CurrentPlayerName}] [战斗系统] 未找到任何符合条件的 抽苔者（可能周围没有怪物或宠物干扰）");
                        }
                    ...
                    new ActionNode(() =>
                    {
                        ...
                        _pauseAntiBotUntil = DateTime.Now.AddMilliseconds(4000);
                        Logger.Write($"[{CurrentPlayerName}] [战斗系统] 准备引怪，暂时屏蔽防检测系统 4 秒...");
                        ExecuteDynamicLua("CastSpellByName('法术反制');");
                        return NodeState.Success;
                    }),
                    ...
                    {
                        // 110目标坐标
                        float pointA_X = 114.02f;
                        float pointA_Y = -232.22f;

                        // 获取当前内存中所有的“抽苔者” (Entry ID: 13022)
                        List<Entry_ID_List> stompers = GetEntitiesByEntryId(_hProcess, 13022);

                        if (stompers.Count == 0)
                            return NodeState.Success; // 没刷怪，直接放行（可根据需要改为Running）
                        ...
                        {
                            shouldLog = true;
                            _debugThrottle_110A = DateTime.Now;
                        }

                        int validCount = 0;        // Z轴合格的怪物数量（过滤掉高处）
                        int combatCount = 0;       // Z轴合格且进战斗的怪物数量
                        float closestDist = 9999f; // 记录最近进战斗怪物的距离
                        float closestZ = 0f;

                        foreach (Entry_ID_List stomper in stompers)
                        {
                            // 1. 过滤Z轴：只在 -20 以下的（地面）目标才有效
                            if (stomper.Z > -20.0f)
                                continue;

                            validCount++;

                            // 2. 检查战斗状态（bit 19）—— 使用 BaseAddress
                            bool inCombat = Unit_Behavioral_State(_hProcess, stomper.BaseAddress, 19);
                            if (!inCombat)
                                continue;
                            ...
                            // 3. 计算到监控点的距离
                            float distToA = (float)Math.Sqrt(
                                Math.Pow(stomper.X - pointA_X, 2) +
                                Math.Pow(stomper.Y - pointA_Y, 2)
                            );
                            ...
                            if (distToA < 3.0f)
                            {
                                Logger.Write($"[{CurrentPlayerName}] [监控系统] 发现目标已到位且进入战斗！距离: {distToA:F2} 码，放行！");
                                _debugThrottle_110A = DateTime.MinValue; // 放行前重置计时
                                return NodeState.Success;
                            }
                        }
                        ...
                        if (shouldLog)
                        {
                            string distStr = combatCount > 0 ? closestDist.ToString("F2") : "无(无进战斗怪物)";
                            Logger.Write($"[{CurrentPlayerName}] [监控系统] 目标点 ({pointA_X}, {pointA_Y}) 等待中... " +
                                         $"抽苔者总数:{stompers.Count}个 | Z轴及格:{validCount}个 | 进战斗数:{combatCount}个 | " +
                                         $"最近进战斗距离:{distStr}码 | 它的Z轴:{closestZ:F2}");
                        }

                        return NodeState.Running; // 没到位或未进战斗，继续等待
                    }),

                    //怪物已经到达目的地
                    new ActionNode(() =>
                    {
                        if (MoveTo(_hProcess, _playerBase, 128.66f, -217.19f, -56.81f))
                        {
                            return NodeState.Success;
                        }
                        return NodeState.Running;
                    ...
                    new ActionNode(() =>
                    {
                        if (MoveTo(_hProcess, _playerBase, 127.62f, -191.23f, -54.91f, 0.5f))
                        {
                            StopMovement(_hProcess, _playerBase);
                            return NodeState.Success;
                        }
                    ...
                    new ActionNode(() =>
                    {
                        CastGroundAOE("暴风雪", 100.7382202f, -203.2011719f, -56.53120422f);  //暴风雪
                        return NodeState.Success;
                    }),
                    ...
                    new ActionNode(() =>
                    {
                        CastGroundAOE("暴风雪", 111.6056519f, -196.6250916f, -55.60294342f);  //暴风雪
                        return NodeState.Success;
                    }),
                    ...
                    {
                        // 110目标坐标
                        float pointA_X = 121.98f;
                        float pointA_Y = -193.66f;
                    ...
                    new ActionNode(() =>
                    {
                        float Point_X = 82.25f;
                        float Point_Y = -204.58f;
                        float Point_Z = -56.82f;
                        ulong nearestGuid = FindUnitGuidByEntryIdAndPosition(_hProcess, 13285, Point_X, Point_Y, Point_Z, 4.0f);
                        if (nearestGuid != 0)
                        {
                            // 以十六进制格式输出，与 Lua 脚本风格保持一致
                            SetFacingByGuid(_playerBase, nearestGuid);
                            Logger.Write($"[{CurrentPlayerName}] [战斗系统] 找到 死亡鞭笞者坐标 X: {Point_X}, Y:{Point_Y}, Z:{Point_Z} 的 GUID: 0x{nearestGuid:X} 并把朝向改成它了");
                        }
                        else
                        {
                            Logger.Write($"[{CurrentPlayerName}] [战斗系统] 未找到任何符合条件的 死亡鞭笞者坐标 X: {Point_X}, Y:{Point_Y}, Z:{Point_Z} （可能周围没有怪物或宠物干扰）");
                        }
                    ...
                    new ActionNode(() =>
                    {
                        if (MoveTo(_hProcess, _playerBase, 127.62f, -191.23f, -54.91f))
                        {
                            return NodeState.Success;
                        }
                        return NodeState.Running;
                    ...
                    new ActionNode(() =>
                    {
                        if (MoveTo(_hProcess, _playerBase, 129.29f, -202.56f, -56.04f))
                        {
                            return NodeState.Success;
                        }
                        return NodeState.Running;
        ...
        public override string OnTick(IntPtr hProcess, int playerBase)
        {
            ...
            if (playerBase != 0)
            {
                ...
                // 1. 职业拦截：不是法师 (8)
                if (currentClassId > 0 && currentClassId != 8)
                {
                    if ((DateTime.Now - _lastClassWarnTime).TotalSeconds >= 5)
                    {
                        Logger.Write($"[{CurrentPlayerName}] [职业拦截] 当前脚本为法师(ID 8)专属！检测到非法职业ID: {currentClassId}，脚本逻辑已锁定！");
                ...
                if (currentLevel <= 0 || currentLevel < 60)
                {
                    ...
                    {
                        Logger.Write($"[{CurrentPlayerName}] [等级拦截] 脚本要求至少60级法师，当前等级: {currentLevel}，脚本逻辑已锁定！");
                ...
                // 技能拦截
                if (!Query_known_skills(_hProcess, 10139)   //六级
                 || !Query_known_skills(_hProcess, 10145)   //六级
                 || !Query_known_skills(_hProcess, 10054)
                 || !Query_known_skills(_hProcess, 10053))
                {
                    if ((DateTime.Now - _lastSkillWarnTime).TotalSeconds >= 5)
                    {
                        var missing = new System.Text.StringBuilder();
                        if (!Query_known_skills(_hProcess, 10139)) missing.Append("造水术");   //六级
                        if (!Query_known_skills(_hProcess, 10145)) missing.Append("造食术");   //六级
                        if (!Query_known_skills(_hProcess, 10054)) missing.Append("制造魔法红宝石");
                        if (!Query_known_skills(_hProcess, 10053)) missing.Append("制造魔法黄水晶");
                        Logger.Write($"[{CurrentPlayerName}] [技能拦截] 缺少必要技能: {missing.ToString().Trim()}，脚本逻辑已锁定！");
                        _lastSkillWarnTime = DateTime.Now;
                    }
                    return "技能不满足要求";
                }
            }
        ...
        ";
            ...
            {
                // [新增核心逻辑] 检查是否处于“屏蔽保护期”
                if (DateTime.Now < _pauseAntiBotUntil)
                {
                    // 还在保护期内，什么都不做，安静地等待保护期结束。
                    // 注意：这里删掉了 return "运行中"; 
                    // 这样当前方法就会继续往下走，执行其他的系统逻辑，而不会跳出整个函数。
                }
                else
                {
                    // 不在保护期，正常执行防作弊判断
                    ...
                    if (canStrafe)
                    {
                        int randomDirection = _antiBotRnd.Next(0, 2);
